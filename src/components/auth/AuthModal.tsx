...
export const AuthModal: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailRegex.test(cleanEmail)) {
      setFormValidationError(t.invalidEmailValidation);
      return;
    }

    if (password.length < 6) {
      setFormValidationError(t.passwordMinLengthValidation);
      return;
    }
    ...
    if (authMode === 'login') {
      ...
      if (!name.trim()) {
        setFormValidationError(t.fullNameRequiredValidation);
        return;
      }
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="••••••••••••"
                className="w-full bg-slate-900 border border-slate-700 rounded-xl pl-9 pr-9 py-2 text-slate-200 focus:outline-hidden focus:border-indigo-500 font-mono text-[11px] transition-all"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3 top-2.5 text-slate-400 hover:text-slate-200 transition-colors"
                title={showPassword ? t.hidePasswordTooltip : t.showPasswordTooltip}
              >
