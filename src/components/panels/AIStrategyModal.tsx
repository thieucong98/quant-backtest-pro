...
export const AIStrategyModal: React.FC = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black/80 backdrop-blur-xs flex items-center justify-center z-50 animate-in fade-in select-none p-2 sm:p-4 font-sans">
        <div className="bg-[#0f1422] border border-amber-500/40 rounded-2xl w-full max-w-md shadow-2xl p-6 text-center text-xs relative flex flex-col items-center">
          ...
          <div className="w-12 h-12 rounded-xl bg-purple-500/20 border border-purple-500/40 flex items-center justify-center text-purple-400 mb-3 shadow-md shadow-purple-500/20">
  ...
  const handleSaveToDatabase = async () => {
    const { isAuthenticated, setAuthModalOpen } = useAuthStore.getState();
    if (!isAuthenticated) {
      setDbSaveMessage(`🔒 ${t.authRequiredDesc}`);
      setAuthModalOpen(true, 'login');
